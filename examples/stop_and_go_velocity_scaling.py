...
def calculate_velocity(min_depth_cm, stop_threshold_cm):
    """ Calculate the velocity based on the distance to the obstacle. """
    if min_depth_cm <= stop_threshold_cm:
        return 0.0  # Stop if too close
    else:
        # Scale velocity linearly between the max and min velocity
        normalized_depth = (min_depth_cm - stop_threshold_cm) / (MAX_DEPTH_M * 100 - stop_threshold_cm)
        velocity = MIN_VELOCITY + (MAX_VELOCITY - MIN_VELOCITY) * normalized_depth
    ...
    for waypoint in waypoints:
        ...
        while True:
            ...
            if min_depth_cm <= depth_threshold:
                ...
                # Calculate velocity based on distance to the obstacle
                velocity_factor = (min_depth_cm / depth_threshold)
                velocity = max(0.1, min(1.0, velocity_factor))  # Scale between 0.1 m/s and 1.0 m/s

                # Continue moving towards the target position
                print(f"Moving to position: North: {target_north} m, East: {target_east} m, Down: {target_down} m with velocity {velocity} m/s")
...
async def run():
    ...
    waypoints = [
        ...
        (11.9, 6.6, -1.0, 45.0, 4),
        (11.9, 6.6, -1.0, 90.0, 2),
        (12.3, 10.0, -1.0, 90.0, 2),
        (12.7, 13.0, -1.0, 90.0, 2),
